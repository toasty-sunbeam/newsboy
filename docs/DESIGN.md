...
6. ✅ **Nightly batch cron (just RSS fetch, no AI yet)**
   - Simple scheduler runs at midnight every day
   - Fetches all enabled RSS feeds
   - Scores articles based on recency, images, content type
   - Selects top 24 articles
   - Creates DailySlot entries (10 at hour 0, then 2/hour)
   - Auto-initializes on server startup via hooks.server.ts
